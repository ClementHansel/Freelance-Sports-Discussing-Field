...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const {
      ...
      userName,
      contentUrl,
    }: ModerationAppealRequest = await req.json();

    console.log("Processing moderation appeal:", {
      contentId,
      contentType,
      userEmail,
    });
    ...
    const { data: content, error: contentError } = await supabase
      ...
      .select(
        ...
        *,
        ${
          contentType === "post"
            ? "topic:topics(title, slug, category:categories(name, slug))"
    ...
    const lastApproval = moderationHistory?.find(
      (h) => h.new_status === "approved" && h.moderator_id
    );
    ...
    const contentTitle =
      contentType === "post" ? content.topic?.title : content.title;
    const categoryName =
      contentType === "post"
        ? content.topic?.category?.name
        : content.category?.name;
    ...
    const adminEmailResponse = await resend.emails.send({
      ...
      html: `
        ...
        <div style="background: #f8f9fa; padding: 15px; border-radius: 5px; margin: 15px 0;">
          ...
          <p><strong>Reporter:</strong> ${userName || "Anonymous"} ${
        userEmail ? `(${userEmail})` : ""
      }</p>
          ${
            contentUrl
              ? `<p><strong>Content URL:</strong> <a href="${contentUrl}">${contentUrl}</a></p>`
              : ""
          }
        </div>
        ...
        <div style="background: #d1ecf1; padding: 15px; border-radius: 5px; margin: 15px 0;">
          ...
          <p><strong>Approved by:</strong> ${
            lastApproval.moderator?.username || "Unknown moderator"
          }</p>
          <p><strong>Approved on:</strong> ${new Date(
            lastApproval.created_at
          ).toLocaleDateString()}</p>
          <p><strong>Reason:</strong> ${
            lastApproval.reason || "No reason provided"
          }</p>
        </div>
        `
            : ""
